    ...
    public class RedisService : IRedisService
    {
        ...
        public async Task<List<int>> GetAllUsersInMatchmakingQueueAsync()
        {
            try
            {
                ...
                foreach (var entry in entries)
                {
                    if (int.TryParse(entry.ToString(), out var userId))
                    {
                        userIds.Add(userId);
                    }
                }
        ...
        public async Task<bool> SaveActiveDuelAsync(Duel duel)
        {
            try
            {
                ...
                var expiry = TimeSpan.FromSeconds(_configuration.GetValue("Redis:ActiveDuelExpiry", 3600));
        ...
        public async Task<List<Duel>> GetUserActiveDuelsAsync(int userId)
        {
            ...
            {
                var db = _redis.GetDatabase();
                const string pattern = $"{ACTIVE_DUELS_KEY}:*";
        ...
        public async Task CompleteAllActiveDuelsAsync()
        {
            try
            {
                ...
                foreach (var duelId in redisActiveKeys.Where(duelId => activeDuelsFromDb.All(d => d.Id != duelId)))
                {
                    try
                    {
