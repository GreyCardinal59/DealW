...
namespace DealW.API.Controllers
{
        ...
        IRedisService redisService) : ControllerBase
    {
        ...
        public async Task<IActionResult> CheckActiveDuel([FromQuery] int? userId = null)
        {
            ...
            var activeDuels = await redisService.GetUserActiveDuelsAsync(userIdValue);
        ...
        public async Task<IActionResult> CreateDuel([FromBody] CreateDuelModel model)
        {
            try
            {
                var duel = await redisService.CreateDuelAsync(model.FirstUserId, model.SecondUserId);
        ...
        public async Task<IActionResult> GetUserDuelHistory([FromQuery] PaginationParams paginationParams, [FromQuery] SortParams? sortParams = null)
        {
            ...
            var duelsQuery = await duelService.GetUserDuelHistoryQueryAsync(userId);
            ...
            return Ok(new
            {
                ...
                hasPreveiwPage = paginatedDuels.HasPreviewPage
            });
        }
        ...
        public async Task<IActionResult> SubmitAnswer(int duelId, int questionOrder, [FromBody] AnswerModel model)
        {
            try
            {
                ...
                var hasSubmitted = await redisService.HasUserSubmittedAnswerAsync(duelId, questionOrder, userId);
                if (hasSubmitted)
                {
                ...
                switch (question.QuestionType)
                {
                    ...
                    case QuestionType.FillBlanks:
                        ...
                        formattedAnswer = model.BlankValues.Count > 0 ? string.Join(";", model.BlankValues) : model.Answer;
                        break;
                ...
                await redisService.SaveUserAnswerAsync(duelId, questionOrder, userId, formattedAnswer);
        ...
        public async Task<IActionResult> CompleteDuel(int duelId)
        {
            try
            {
                await redisService.CompleteDuelAsync(duelId);
        ...
        public async Task<IActionResult> CompleteAllDuels()
        {
            try
            {
                await redisService.CompleteAllActiveDuelsAsync();
            ...
            try
            {
                ...
                var result = new
                {
                    ...
                    questions = duel.DuelQuestions?.OrderBy(dq => dq.QuestionOrder).Select(dq => new
                    {
                        ...
                        options = dq.Question?.QuestionType == QuestionType.MultipleChoice && duel.IsCompleted ? 
                            dq.Question.GetOptions() : null,
                        template = dq.Question?.QuestionType == QuestionType.FillBlanks && duel.IsCompleted ? 
                            dq.Question.GetBlanksTemplate() : null,
                        blanks = dq.Question?.QuestionType == QuestionType.FillBlanks && duel.IsCompleted ? 
        ...
        private async Task<List<object>> GetDuelQuestionsInfoAsync(Duel duel, int userId)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                var question = await redisService
                    .GetDuelQuestionAsync(duel.Id, i);
                
                var hasSubmitted = await redisService
                    .HasUserSubmittedAnswerAsync(duel.Id, i, userId);
        ...
        public async Task<IActionResult> GetDuelStatus(int duelId)
        {
            try
            {
                ...
                var duel = await duelService.GetDuelByIdAsync(duelId);
